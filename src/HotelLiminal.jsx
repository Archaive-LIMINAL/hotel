...
const RULES = [
  ...
  "2. 새벽 3시부터 5시 사이에는 다른 투숙객을 위해 복도 내 이동을 최소화하여 주십시오.",
...
function useSound() {
  ...
  const stop = useCallback((key) => {
    try {
      ...
      if (a) { a.pause(); a.currentTime = 0; delete audioRefs.current[key]; }
    } catch (e) {}
  }, []);

  const stopAll = useCallback(() => {
    try { Object.keys(audioRefs.current).forEach(k => stop(k)); } catch (e) {}
  }, [stop]);

  const startBg = useCallback(() => {
    try {
      if (bgRef.current) return;
      bgRef.current = new Audio(SND.rain);
...
function ElevatorDisplay({sequence,onComplete,initialFloor,autoStart}){
  const[idx,setIdx]=useState(-1);const[floor,setFloor]=useState(initialFloor||"—");const[status,setStatus]=useState("");const[arrow,setArrow]=useState("");const[anim,setAnim]=useState("");const started=useRef(false);
  useEffect(()=>{setIdx(-1);setFloor(initialFloor||"—");setStatus("");setArrow("");started.current=false;},[sequence]);
  useEffect(()=>{if(autoStart&&sequence&&sequence.length>0&&!started.current){started.current=true;setIdx(0);}},[autoStart,sequence]);
  useEffect(()=>{if(idx<0||!sequence||idx>=sequence.length)return;const item=sequence[idx];if(item.type==="floor"){setFloor(String(item.value));setStatus("");setArrow(item.dir==="up"?"▲":"▼");setAnim("floor-anim-"+(item.dir||"down"));setTimeout(()=>setAnim(""),300);}else if(item.type==="pause"){setArrow("");setStatus("");setAnim("pause-pulse");}else if(item.type==="door_open"){setStatus("열림");}else if(item.type==="door_close"){setStatus("닫힘");}else if(item.type==="arrive"){setFloor(String(item.value));setArrow("");setStatus("");setAnim("");}const t=setTimeout(()=>{if(idx<sequence.length-1)setIdx(idx+1);else{onComplete&&onComplete();}},item.duration||1300);return()=>clearTimeout(t);},[idx,sequence,onComplete]);
...
/* ═══════════════════════════════════════════
   RULES SCREEN (supports multiple highlights + multi-page)
   ═══════════════════════════════════════════ */
function RulesScreen({page,onNext,highlightRules}){
  ...
  return(
    <div className="rules-overlay"><div className="rules-paper">
      ...
      <div className="rules-footer"><button className="rules-btn" onClick={onNext}>{hl.length>0?"다시 시작":page===1?"다음 페이지":"규칙서를 내려놓는다"}</button></div>
    </div></div>
  );
}

/* ═══════════════════════════════════════════
   MULTI-PAGE DEATH RULES VIEWER
   ═══════════════════════════════════════════ */
function DeathRulesViewer({rules,onRestart}){
  const hasP1=rules.some(r=>r<10),hasP2=rules.some(r=>r>=10);
  const needsBoth=hasP1&&hasP2;
  const[page,setPage]=useState(hasP1?1:2);
  if(needsBoth){
    return page===1?
      <RulesScreen page={1} highlightRules={rules} onNext={()=>setPage(2)}/>:
      <RulesScreen page={2} highlightRules={rules} onNext={onRestart}/>;
  }
...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
const INIT={
  heardKnock:false,knockSceneSeen:false,
  hasCardKey:false,metLobbyPerson:false,lobbyPersonGone:false,
  entranceVisited:false,entranceWomanGone:false,
  arrivedFake7:false,fake7CorrVisited:false,fake7ElevCalled:false,
  stairsFrom7Done:false,
  floor3Arrived:false,floor3Fled:false,floor3Hidden:false,
  soundLocs:[],eventPhase:0,
};
...
export default function HotelLiminal(){
  ...
  const[accumReasons,setAccumReasons]=useState([]); // array of rule indices
  const[deathText,setDeathText]=useState("");
  const[deathRules,setDeathRules]=useState([]); // array of rule indices
  ...
  const accumDie=useCallback(()=>{snd.stopAll();snd.stopBg();tr(()=>{setDeathRules([...accumReasons,7]);setPhase(PHASE.ACCUM_DEATH);setBrightness("normal");});},[tr,accumReasons,snd]);
  const addAccum=useCallback((ruleIdx)=>{setAccumReasons(p=>[...new Set([...p,ruleIdx])]);},[]);
  ...
  const moveTo=useCallback((dest)=>{
    snd.play("step",{volume:0.4});
    if(flags.eventPhase===0&&flags.heardKnock&&dest==="7_room_right_b"&&!flags.knockSceneSeen){
    ...
    if(flags.fake7ElevCalled&&(dest==="f7_cor_upper"||dest==="f7_cor_lower")){die("뒤에서 무언가가 다가왔다.",8);return;}
    if(flags.eventPhase===6&&flags.floor3Arrived&&!flags.floor3Fled){
      if(flags.soundLocs.includes(dest)){die("뒤에서 무언가가 다가왔다.",1);return;}
    }
    if(flags.eventPhase===6&&flags.floor3Fled&&(dest==="3_room_left_a"||dest==="3_room_left_b")){
      snd.play("powercut",{volume:0.6});
      tr(()=>{setLoc(dest);setBrightness("blackout");setNavOff(true);setShowElev(false);setCurFloor(3);
        ...
        setChoices([
          ...
          {text:"복도 끝에 몸을 숨긴다",action:()=>{tr(()=>{setBrightness("blackout");setT("복도 끝에 웅크렸다.\n\n완전한 어둠.\n\n발소리가 지나간다.\n\n...\n\n불이 다시 켜졌다.");uF({floor3Hidden:true,eventPhase:7,soundLocs:[]});snd.stop("chase");setBrightness("normal");setChoices([]);setNavOff(false);});}},
        ]);
      });return;
    }
  ...
  const handleArr=useCallback((l)=>{
    if(l==="7_room_right_b"&&flags.eventPhase===0&&!flags.heardKnock){setBrightness("normal");setT("712호 객실 앞이다.");setChoices([]);setNavOff(false);return;}
    if(l==="7_corridor_lower"&&flags.eventPhase===0&&!flags.heardKnock){
      uF({heardKnock:true,soundLocs:["7_room_right_b"]});setBrightness("normal");
      snd.play("knock",{volume:0.7});
      setT("복도를 걷는다.\n\n반대쪽에서 무언가 문을 두드리는 소리가 들린다.");setChoices([]);setNavOff(false);advTime(1);return;
    }
    if(l==="7_elev_area"&&flags.eventPhase===0&&flags.heardKnock&&!flags.stairsFrom7Done){
    ...
    if(l==="1_lobby"){
      if(!flags.hasCardKey&&!flags.metLobbyPerson){
      ...
      if(flags.hasCardKey&&!flags.lobbyPersonGone){
        uF({lobbyPersonGone:true,soundLocs:["1_entrance"]});setBrightness("dimNormal");setT("로비로 돌아왔다.\n\n아까 소파에 앉아있던 사람은 사라졌다.");setChoices([]);setNavOff(false);return;
      }
      setBrightness("dimNormal");setT("새벽의 로비.\n\n조용하다.");
      if(flags.lobbyPersonGone&&flags.entranceVisited&&!flags.entranceWomanGone)uF({soundLocs:["1_entrance"]});
      setChoices([]);setNavOff(false);return;
    }
    if(l==="1_entrance"){
      if(!flags.entranceVisited){uF({entranceVisited:true});setBrightness("dimNormal");
        snd.play("knock",{volume:0.5});
        setT("정문 앞에 섰다.\n\n잠겨있다.\n\n밖에서 누군가 유리문을 두드린다.\n\n\"저기요...\n카드키를 잃어버려서 들어갈 수가 없어요.\n문 좀 열어주실 수 있나요...?\"");
        setChoices([{text:"문을 열어준다",action:()=>die("밖에는 아무도 없었다.",16)}]);setNavOff(false);return;
      }
      if(flags.hasCardKey&&!flags.entranceWomanGone){uF({entranceWomanGone:true,soundLocs:[]});setT("다시 정문 앞에 왔다.\n\n아무도 없다.\n\n아까 밖에 있던 여자도 보이지 않는다.");setChoices([]);setNavOff(false);return;}
      setT("잠긴 정문.\n\n밖은 어둡다.");setChoices([]);setNavOff(false);return;
    }
    if(l==="1_front_desk"){
      if(!flags.hasCardKey){setBrightness("dimNormal");setT("프론트 데스크.\n\n아무도 없다.\n\n카운터 위에 벨이 놓여있다.");
        setChoices([
          {text:"벨을 누른다",action:()=>{snd.play("bell",{volume:0.6});addAccum(11);getKey();}},
    ...
    if(l==="1_elev_area"){
      if(flags.hasCardKey){setBrightness("normal");setShowElev(false);setT("엘리베이터 앞에 섰다.");setChoices([{text:"엘리베이터를 부른다",action:()=>startElevUp()}]);setNavOff(false);return;}
      setBrightness("normal");setT("엘리베이터 앞.\n\n먼저 카드키를 받아야 할 것 같다.");setChoices([]);setNavOff(false);return;
    }
    if(l==="f7_elev"){
      if(!flags.arrivedFake7){uF({arrivedFake7:true,eventPhase:4});setBrightness("normal");setCurFloor(7);setT("7층에 도착했다.\n\n엘리베이터 앞이다.");setChoices([]);setNavOff(false);return;}
      if(flags.fake7CorrVisited&&!flags.fake7ElevCalled){
        setBrightness("normal");setShowElev(true);setElevInitFloor("1");setElevSeq(null);setElevAuto(false);
        uF({soundLocs:["f7_cor_upper","f7_cor_lower"]});setT("복도 쪽에서 소리가 점점 다가오고 있다.");
        snd.play("chase",{volume:0.4,loop:true});
        setChoices([{text:"엘리베이터를 부른다",action:()=>callFake7Elev()}]);setNavOff(false);return;
      }
      setBrightness("normal");setT("엘리베이터 앞.");setChoices([]);setNavOff(false);return;
    }
    if(l==="f7_cor_upper"||l==="f7_cor_lower"){
      if(!flags.fake7CorrVisited)uF({fake7CorrVisited:true});
    ...
    if(l==="3_elev_area"&&flags.eventPhase===6){
      if(!flags.floor3Arrived){
        uF({floor3Arrived:true,soundLocs:["3_corridor_lower","stairs"]});
        setBrightness("dark");setShowElev(true);setElevInitFloor("7");setElevSeq(null);setElevAuto(false);
        snd.play("knock",{volume:0.5});
        setT("3층에 도착했다.\n\n근처에서 노크 소리가 들린다.");
        setChoices([{text:"엘리베이터를 부른다",action:()=>call3FElev()}]);setNavOff(false);return;
      }
      setBrightness("dark");setT("3층 엘리베이터 앞.");setChoices([]);setNavOff(false);return;
    }
    if(l==="3_corridor_upper"&&flags.eventPhase===6&&flags.floor3Fled){
      uF({soundLocs:["3_elev_area"]});setBrightness("dark");setT("상부 복도로 왔다.\n\n엘리베이터 쪽에서 소리가 들린다.");setChoices([]);setNavOff(false);return;
    }
    if(flags.eventPhase===7&&flags.floor3Hidden){
      if(l==="3_corridor_upper"||l==="3_corridor_lower"){setBrightness("normal");setT("복도.\n\n조용하다.");setChoices([]);setNavOff(false);return;}
      if(l==="3_elev_area"){setBrightness("normal");setShowElev(false);setT("엘리베이터 앞에 도착했다.");
        setChoices([{text:"엘리베이터를 부른다",action:()=>startFinalElev()},{text:"비상계단으로 올라간다",action:()=>startFinalStairs()}]);setNavOff(false);return;}
    }
    const ld=LOCATIONS[l];if(ld){setT(ld.name+".");setChoices([]);setNavOff(false);}
  },[flags,uF,advTime,die,tr,snd,addAccum]);
  ...
  const startElevDown=useCallback(()=>{
    uF({eventPhase:1,soundLocs:[]});snd.play("elev",{volume:0.5});snd.startBg();
    tr(()=>{setLoc("elev_inside");setBrightness("normal");setNavOff(true);setShowElev(true);setElevInitFloor("7");setElevAuto(true);
      setElevSeq([{type:"floor",value:7,dir:"down",duration:1300},{type:"floor",value:6,dir:"down",duration:1300},{type:"floor",value:5,dir:"down",duration:1300},{type:"floor",value:3,dir:"down",duration:1300},{type:"floor",value:2,dir:"down",duration:1300},{type:"arrive",value:1,duration:1000}]);
      setCurFloor("—");setT("엘리베이터에 탑승했다.\n\n1층 버튼을 누른다.");setChoices([]);});
  },[tr,uF,snd]);

  const onElevDownDone=useCallback(()=>{snd.play("elev",{volume:0.5});setCurFloor(1);setT("도착.");setChoices([{text:"내린다",action:()=>{tr(()=>{setPhase(PHASE.GAME);setLoc("1_lobby");setCurFloor(1);setShowElev(false);handleArr("1_lobby");});}}]);},[tr,handleArr,snd]);

  const startElevUp=useCallback(()=>{
    snd.play("elev",{volume:0.5});
    tr(()=>{setLoc("elev_inside");setBrightness("normal");setNavOff(true);setShowElev(true);setElevInitFloor("1");setElevAuto(true);
      setElevSeq([{type:"floor",value:1,dir:"up",duration:1300},{type:"floor",value:2,dir:"up",duration:1300},{type:"floor",value:3,dir:"up",duration:1300},{type:"floor",value:5,dir:"up",duration:1300},{type:"floor",value:6,dir:"up",duration:1300},{type:"arrive",value:7,duration:1000}]);
      setCurFloor("—");setT("엘리베이터에 탑승했다.\n\n7층 버튼을 누른다.");setChoices([]);});
  },[tr,snd]);

  const onElevUpDone=useCallback(()=>{snd.play("elev",{volume:0.5});setCurFloor(7);setT("도착.");setChoices([{text:"내린다",action:()=>{tr(()=>{setPhase(PHASE.GAME);setLoc("f7_elev");setCurFloor(7);setShowElev(false);handleArr("f7_elev");});}}]);},[tr,handleArr,snd]);

  const startStDown7=useCallback(()=>{
    uF({soundLocs:[]});
    tr(()=>{setLoc("stairs");setBrightness("normal");setNavOff(true);setShowElev(false);setCurFloor(7);
      setT("비상계단에 들어섰다.\n\n내려가기 시작한다.");
      setChoices([{text:"내려간다",action:()=>{
        snd.play("powercut",{volume:0.5});
        tr(()=>{setBrightness("blackout");setCurFloor("—");setT("불이 꺼졌다.");
          setChoices([{text:"멈춘다",action:()=>die("발소리가 멈췄다.",18)},{text:"계속 내려간다",action:()=>stD(1)}]);});
      }}]);});
  },[tr,die,uF,snd]);
  ...
  const callFake7Elev=useCallback(()=>{
    uF({fake7ElevCalled:true});setElevAuto(true);snd.play("elev",{volume:0.5});
    setElevSeq([{type:"floor",value:1,dir:"up",duration:1300},{type:"floor",value:2,dir:"up",duration:1300},{type:"floor",value:3,dir:"up",duration:1300},{type:"floor",value:4,dir:"up",duration:1300},{type:"pause",duration:1500},{type:"door_open",duration:1500},{type:"door_close",duration:1200},{type:"floor",value:5,dir:"up",duration:1300},{type:"floor",value:6,dir:"up",duration:1300},{type:"arrive",value:7,duration:1000}]);
    setChoices([]);
  },[uF,snd]);

  const onFake7ElevDone=useCallback(()=>{
    snd.play("elev",{volume:0.5});
    setT("엘리베이터가 도착했다.");setChoices([{text:"엘리베이터에 탑승한다",action:()=>die("혼자가 아니었다.",14)}]);
    timerRef.current=setTimeout(()=>{die("벗어나지 못했다.",5);},5000);setNavOff(false);
  },[die,snd]);

  const startSt7AD=useCallback(()=>{
    if(timerRef.current){clearTimeout(timerRef.current);timerRef.current=null;}
    uF({soundLocs:[]});snd.stop("chase");
  ...
  const st7AD=useCallback((step)=>{
    ...
    else{snd.play("powercut",{volume:0.5});tr(()=>{setBrightness("blackout");setCurFloor("—");setT("불이 꺼졌다.");setChoices([{text:"내려간다",action:()=>st7ADdark(1)}]);});}
  },[tr,snd]);
  ...
  const call3FElev=useCallback(()=>{
    setElevAuto(true);snd.play("elev",{volume:0.5});snd.play("chase",{volume:0.4,loop:true});
    setElevSeq([{type:"floor",value:7,dir:"down",duration:1300},{type:"floor",value:6,dir:"down",duration:1300},{type:"floor",value:5,dir:"down",duration:1300}]);
    setT("층수 표시가 움직인다.\n\n발소리가 점점 다가온다.");setChoices([]);setNavOff(true);
  },[snd]);

  const on3FElevDone=useCallback(()=>{
    uF({floor3Fled:true,soundLocs:["3_corridor_lower","3_elev_area"]});setShowElev(false);
    setT("발소리가 바로 뒤까지 왔다.");setChoices([]);setNavOff(false);
  },[uF]);

  const startFinalElev=useCallback(()=>{
    snd.stop("chase");snd.play("elev",{volume:0.5});
    tr(()=>{setLoc("elev_inside");setBrightness("normal");setNavOff(true);setShowElev(true);setElevInitFloor("3");setElevAuto(true);
      setElevSeq([{type:"floor",value:3,dir:"up",duration:1300},{type:"floor",value:4,dir:"up",duration:1800},{type:"floor",value:5,dir:"up",duration:1300},{type:"floor",value:6,dir:"up",duration:1300},{type:"arrive",value:7,duration:1000}]);
      setCurFloor(3);setT("엘리베이터 앞에 섰다.\n\n부른다.\n\n정상적으로 도착했다.\n\n탑승한다.\n\n7층을 누른다.");setChoices([]);});
  },[tr,snd]);

  const onFinalElevDone=useCallback(()=>{
    snd.play("elev",{volume:0.5});setCurFloor(7);setT("도착.");
    setChoices([{text:"내린다",action:()=>{
      tr(()=>{setPhase(PHASE.GAME);setShowElev(false);
  ...
  const startFinalStairs=useCallback(()=>{
    ...
    tr(()=>{setLoc("stairs");setBrightness("normal");setNavOff(true);setShowElev(false);setCurFloor(3);
      setT("비상계단으로 올라간다.\n\n4...\n5...\n6...\n7.");
      setChoices([{text:"7층으로 나간다",action:()=>{
  ...
  const onElevDone=useCallback(()=>{
    if(flags.eventPhase===1)onElevDownDone();
    else if(flags.eventPhase===2&&!flags.arrivedFake7)onElevUpDone();
    else if(flags.eventPhase===4)onFake7ElevDone();
    else if(flags.eventPhase===6)on3FElevDone();
    else if(flags.eventPhase===7)onFinalElevDone();
    else onElevUpDone();
  },[flags,onElevDownDone,onElevUpDone,onFake7ElevDone,on3FElevDone,onFinalElevDone]);
  ...
  const getNav=()=>{
    ...
    const ld=LOCATIONS[loc];if(!ld)return{left:[],right:[]};
    let conns=[...ld.connections];
    if(loc==="f7_elev"&&flags.fake7CorrVisited)conns.push({to:"__stairs_f7",dir:"right"});
    if(loc==="3_elev_area"&&(flags.eventPhase===6||flags.eventPhase===7))conns.push({to:"__stairs_3f",dir:"right"});
    const mkNav=c=>({id:c.to,label:(c.to==="__stairs_f7"||c.to==="__stairs_3f")?"비상계단":(LOCATIONS[c.to]?.short||c.to),hasSound:flags.soundLocs.includes(c.to)||(c.to==="__stairs_3f"&&flags.soundLocs.includes("stairs"))});
    return{left:conns.filter(c=>c.dir==="left").map(mkNav),right:conns.filter(c=>c.dir==="right").map(mkNav)};
  };

  const nav=getNav();
  const handleNavClick=(id)=>{
    if(id==="__stairs_f7")startSt7AD();
    else if(id==="__stairs_3f"){if(flags.eventPhase===7)startFinalStairs();else die("뒤에서 무언가가 다가왔다.",1);}
    else moveTo(id);
  };
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .nav-area{padding:.5rem 1.5rem 1rem;display:flex;justify-content:space-between;border-top:1px solid rgba(255,255,255,.04);position:relative;z-index:60;min-height:3rem;background:#08080a}
        .nav-col{display:flex;flex-direction:column;gap:.25rem;min-width:40%}
        .nav-col-left{align-items:flex-start}
        .nav-col-right{align-items:flex-end}
        .nav-btn{display:flex;align-items:center;gap:.3em;background:none;border:none;color:#5a5550;font-family:'Noto Sans KR',sans-serif;font-size:.7rem;font-weight:300;cursor:pointer;padding:.35em .2em;transition:color .2s;letter-spacing:.05em;white-space:nowrap}
        .nav-btn:hover{color:#a09888}
        .nav-arrow{font-size:.8rem;color:#4a4540}
        .nav-sound{color:#8a4030;font-size:.55rem;animation:soundPulse 1s ease infinite}
        ...
        <div className={`game-frame ${!fade?"fading":""}`}>
          {phase===PHASE.TITLE&&(
            <div className="title-screen">
              ...
              <div className="title-tagline">규칙을 기억하세요</div>
              <button className="title-start" onClick={()=>{snd.startBg();tr(()=>setPhase(PHASE.RULES));}}>체크인</button>
            </div>
          )}
          {phase===PHASE.RULES&&<RulesScreen page={1} highlightRules={[]} onNext={()=>tr(()=>setPhase(PHASE.RULES_P2))}/>}
          {phase===PHASE.RULES_P2&&<RulesScreen page={2} highlightRules={[]} onNext={()=>{
            tr(()=>{setPhase(PHASE.GAME);setLoc("7_room_right_b");setCurFloor(7);
              setT("객실 712호.\n\n테이블 위에 놓여있던 규칙서를 대충 훑어봤다.\n\n침대에 누웠다.\n\n...\n\n배가 고프다.\n\n시계를 보니 새벽 2시 47분.\n\n편의점이라도 다녀와야겠다.");
              setChoices([{text:"객실을 나선다",action:()=>{tr(()=>{setLoc("7_room_right_b");setT("복도로 나왔다.\n\n뒤를 돌아보니 문이 닫혀 있다.\n\n주머니를 뒤졌다.\n\n카드키가 없다.\n\n방 안에 두고 나온 것 같다.");setChoices([]);setNavOff(false);advTime(1);});}}]);
              setNavOff(true);});
          }}/>}
            ...
            <>
              ...
              <div className="game-content">
                <div className="scene-header">
                  <div className="header-left"><FloorIndicator floor={curFloor}/><span className="scene-location">{LOCATIONS[loc]?.name||loc}</span></div>
                  <span className="scene-time">{gameTime}</span>
                </div>
                <div className="scene-divider"/>
                {showElev&&elevSeq!==undefined&&<ElevatorDisplay sequence={elevSeq||[]} onComplete={onElevDone} initialFloor={elevInitFloor} autoStart={elevAuto}/>}
